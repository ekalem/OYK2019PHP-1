...
Bu komut ile sistemimize ubuntu depolarında bulunan her paketi kurabiliriz. 

```sudo apt remove paket-ismi```
Bu komut ile istediğimiz paketi silebiliriz. 

```sudo apt update```
Paket listesini günceller.

```sudo apt upgrade```
Sistemde kurulu paketleri günceller.
...
### Debian Nedir?
**Debian**, günümüze kadar ulaşmış ve hala sayısız kullanıcı tarafından kullanılan GNU/Hurd, *GNU/Linux* gibi farklı çekirdek seçeneklerine bağlı özgür bir Linux dağıtımıdır. 
